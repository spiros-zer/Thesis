...
data = cell(5,1);
for k = 1:3
    data{k} = load(['RICIAN.CNR' num2str(10*(k-1)) '.mat']);
end
% BER %
...
ylabel('Phase (rad)');
for k=2:3
    plot(angle(data{k}.IR_CARRIER_IM), 'k');
    plot(data{k}.phiMEAN_IM)
end
legend('CNR_{IR phase}=0dB','CNR_{approx}=0dB','CNR_{IR phase}=10dB','CNR_{approx}=10dB','CNR_{IR phase}=20dB','CNR_{approx}=20dB');
hold off;